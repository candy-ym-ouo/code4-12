...
- CSV、完整工作区 JSON 导出。
- 仪表盘、审计日志和健康检查。
- 材料替代推荐引擎：按工艺、颜色、单位与库存兼容性评分排序，硬条件不过的候选保留拒绝原因；规则版本化，改版后可按材料或全量重算；人工锁定的结果在任何重算中都不会被覆盖。
...
- 计划用料不扣库存，实际消耗才扣库存。
- 替代推荐的规则集只增不改：调整参数生成新版本并归档旧版本，推荐行记录生成时的规则版本。
- 替代重算只重建未锁定结果；人工锁定行（含拒绝行）原样保留，不重新评分或排名。
- 兼容关系 `BLOCKED` 是硬拒绝，`ALLOWED` 只在加权评分上加分，不绕过任何硬条件。
